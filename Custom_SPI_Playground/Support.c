...
CyBool_t ApplicationIsActive(void)
{
	// Check run time flags to discover if application is running
	uint32_t ActualEvents;
	CyU3PEventGet(&glApplicationEvent, 0, CYU3P_EVENT_OR, &ActualEvents, 0);
	return ((ActualEvents & (DATA_BUFFER_AVAILABLE | DATA_BUFFER_PROCESSED)) > 0);
}
